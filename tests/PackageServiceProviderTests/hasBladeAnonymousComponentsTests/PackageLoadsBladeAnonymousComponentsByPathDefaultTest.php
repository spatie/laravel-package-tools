...
trait PackageLoadsBladeAnonymousComponentsByPathDefaultTest
{
    public function configurePackage(Package $package)
    {
        ...
        if (! is_before_laravel_version(App::version(), '9.44.0')) {
            $package
                ->loadsViewsByPath()
                ->loadsBladeAnonymousComponentsByPath();
        }
    }
}

uses(PackageLoadsBladeAnonymousComponentsByPathDefaultTest::class);

it("can load the blade components by path", function () {
    $content = view('package-tools::component-test-anonymous-default')->render();
